...
public class Main {
    ...
    public void test_monitor_schedule_logic() {
        ...
        //while (Math.random() != 0) {
        String tradeFinnalString = test_trade_pay(tradeWithHBService);
            Utils.sleep(5 * 1000);
        //}

        // 满足退出条件后可以调用shutdown优雅安全退出
        if(tradeFinnalString.equals("SUCCESS")){
        	log.debug("支付成功,安全退出");
        	demoRunner.shutdown();
...
//        sysTradeInfoList.add(SysTradeInfo.newInstance("00000005", 4.1, HbStatus.X));
        ...
        String appAuthToken = "2088102175517405";//根据真实值填写
        ...
        AlipayHeartbeatSynRequestBuilder builder = new AlipayHeartbeatSynRequestBuilder()
            ...
            .setNetworkType("LAN").setProviderId("2088102175517405") // 设置系统商pid
    ...
    public String test_trade_pay(AlipayTradeService service) {
        ...
        String authCode = "286210506436921005"; // 条码示例，286648048691290423
        ...
        double timeConsume = 5.23;
        switch (result.getTradeStatus()) {
            case SUCCESS:
                log.info("支付宝支付成功: )");
                resultString = HbStatus.S;
            ...
            case FAILED:
                log.error("支付宝支付失败!!!");
                resultstring = "FAILED";
                resultString = HbStatus.F;
                break;

            case UNKNOWN:
                log.error("系统异常，订单状态未知!!!");
                resultstring = "UNKNOWN";
                resultString = HbStatus.X;
                break;
        ...
        SysTradeInfo sysTradeInfo = SysTradeInfo.newInstance(outTradeNo,timeConsume,resultString);
        HbQueue.offer(sysTradeInfo);//当面付程序每执行完一笔交易后将交易结果保存在临时队列
        return resultstring;
    }
